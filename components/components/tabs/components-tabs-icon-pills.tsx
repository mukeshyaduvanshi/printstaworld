...
<Tab.Group>
    <Tab.List className="mt-3 flex flex-wrap gap-2">
        <Tab as={Fragment}>
            {({ selected }) => (
                <button
                    className={\`\${selected ? 'bg-warning text-white outline-hidden!' : ''} before:inline-block -mb-[1px] flex items-center rounded-sm p-3.5 py-2 hover:bg-warning hover:text-white\`}>
                    <svg>...</svg>
                    Home
                </button>
            )}
